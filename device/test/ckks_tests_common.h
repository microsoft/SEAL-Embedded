...
/**
(Pseudo) ckks decode. 'values_decoded' and 'pt' may share the same starting address for
in-place computation (see: ckks_decode_inpl)
...
If 'SE_INDEX_MAP_LOAD' is defined, index_map must constain space for n uint16_t elements.

@param[in]  pt              Plaintext to decode
@param[in]  values_len      True number of entries in initial values message. Must be <=
n/2
...
/**
Pseudo-decrypts a ciphertext in place (for a particular prime component). Currently only
works if 'small_s' is 0. 'c0' and 'pt' may share the same starting address for in-place
computation (see: ckks_decrypt_inpl).
...
/**
Pseudo-decrypts a ciphertext in place (for a particular prime component). Currently only
works if small_s is 0. Note that this function will also corrupt the memory pointed to by
c1.
...
/**
Checks that the pseudo-decoding and pseudo-decryption of a ciphertext is correct (for a
particular prime ciphertext component). Currently only works if 'small_s' is 0.
...
@param[in]  s           Secret key. Must be in expanded form!
@param[in]  small_s     If true, secret key is in small form; Else, is in expanded form
@param[in]  pte_calc    Plaintext + error (in NTT form) (output by ckks_encode for
testing)
@param[in]  index_map   [Optional]. If passed in, can avoid 1 flash read
@param[in]  parms       Parameters instance
@param      temp	    Scratch space
