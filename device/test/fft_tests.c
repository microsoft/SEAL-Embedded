...
Space req: 'res' must constain space for n double complex elements, where each input
polynomial consists of n double complex elements (each).
...
/**
Pointwise multiplies two complex-valued polynomials. 'a' and 'res' starting address may
overlap for in-place computation (see: pointwise_mult_inpl_complex)
...
void test_fft_mult_helper(size_t degree, double complex *v1, double complex *v2,
                          double complex *v_exp, double complex *temp, double complex *roots)
{
    size_t n    = degree;
...
void test_fft(void)
{
    ...
    size_t start_idx   = 0;
    double complex *v1 = &(mempool[start_idx]);
    start_idx += n;
    double complex *v2 = &(mempool[start_idx]);
    start_idx += n;
    double complex *v_exp = &(mempool[start_idx]);
    start_idx += n;
    double complex *temp = &(mempool[start_idx]);
    start_idx += n;
    double complex *roots = roots_size ? &(mempool[start_idx]) : 0;
    start_idx += roots_size;
    ...
    for (size_t testnum = 0; testnum < 15; testnum++)
    {
        ...
        switch (testnum)
        {
            ...
            case 2:                                       // {0, 1, 2, 3, ...}
                for (size_t i = 0; i < n; i++) { v1[i] = (double complex)_complex(i, 0); }
                break;
            case 3:
                for (size_t i = 0; i < n; i++)
                {
                    v1[i] = (double complex)(gen_double_eighth(10), 0);
                }
                break;
            case 4:
                for (size_t i = 0; i < n; i++)
                {
                    v1[i] = (double complex)(gen_double_quarter(100), 0);
                }
                break;
            case 5:
                for (size_t i = 0; i < n; i++)
                {
                    v1[i] = (double complex)(gen_double_half(-100), 0);
                }
                break;
            case 6:
                for (size_t i = 0; i < n; i++) { v1[i] = (double complex)(gen_double(1000), 0); }
                break;
            case 7:  // {1, 0, 0, ...} * {2, 2, 2, ...} = {2, 2, 2, ..., 0, 0, ...}
                v1[0] = (double complex)_complex(1, 0);
            ...
            case 10:  // {1, 1, 1, ...} * {2, 2, 2, ...} = {2, 4, 8, ..., 4, 2, 0}
                set_double_complex(v1, n / 2, 1);
                set_double_complex(v2, n / 2, 2);
                for (size_t i = 0; i < n / 2; i++)
                {
                    v_exp[i] = (double complex)_complex(2 * (i + 1), 0);
                }
            ...
            case 11:
                for (size_t i = 0; i < n / 2; i++)
                {
                    v1[i] = (double complex)_complex(gen_double_eighth(pow(10, 1)), 0);
                    v2[i] = (double complex)_complex(gen_double_eighth(pow(10, 1)), 0);
                }
                break;
            case 12:
                for (size_t i = 0; i < n / 2; i++)
                {
                    v1[i] = (double complex)_complex(gen_double_quarter(-pow(10, 2)), 0);
                    v2[i] = (double complex)_complex(gen_double_quarter(-pow(10, 2)), 0);
                }
                break;
            case 13:
                for (size_t i = 0; i < n / 2; i++)
                {
                    v1[i] = (double complex)(gen_double_half(pow(10, 3)), 0);
                    v2[i] = (double complex)(gen_double_half(pow(10, 3)), 0);
                }
                break;
            case 14:
                for (size_t i = 0; i < n / 2; i++)
                {
                    v1[i] = (double complex)(gen_double(pow(10, 6)), 0);
                    v2[i] = (double complex)(gen_double(pow(10, 6)), 0);
                }
                break;
        }
        if (testnum < 7)
