...
Size req: If SE_NTT_OTF is not defined, 'ntt_roots' must contain space for roots according
to NTT type chosen. If seed != NULL, seed must be SE_PRNG_SEED_BYTE_COUNT long.

@param[in]     parms           Parameters set by ckks_setup
@param[in]     s_small         Secret key in small form
@param         ntt_roots       [Optional]. Scratch space to load ntt roots. Ignored if
SE_NTT_OTF is chosen.
...
@param[out]    s_save          Secret key modulo the current modulus (in ntt form). Useful
for testing.
...
/**
Initializes values for a single full asymmetric CKKS encryption. Samples the errors (w/o
respect to any prime), as well as the ternary polynomial 'u'. Should be called once per
encode-encrypt sequence (just after ckks_encode_base).
...
@param[in]     parms          Parameters set by ckks_setup
@param[in]     seed           [Optional.] Seed to seed 'prng' with.
@param[in,out] prng           PRNG instance needed to generate error and ternary
polynomials.
...
/**
Encodes and asymmetrically encrypts a vector of values using CKKS, for the current modulus
prime. Optionally returns some additional values useful for testing, if
SE_DISABLE_TESTING_CAPABILITY is not defined.
...
@param      ntt_roots      [Optional]. Scratch space to load ntt roots. Ignored if
SE_NTT_OTF is chosen.
@param[out] ntt_u_e1_pte   Scratch space. Out: m + e0 in reduced and ntt form (useful for
testing).
@param[out] ntt_u_save     [Optional, ignored if NULL]. Expanded and ntt form of u (useful
for testing)
@param[out] ntt_e1_save    [Optional, ignored if NULL]. Reduced and ntt form of e1 (useful
for testing)
...
/**
Updates parameters to next prime in modulus switching chain for asymmetric CKKS
encryption. Also reduces ternary polynomial to next prime modulus if used in expanded form
(compressed form u will be reduced later).

@param[in,out] parms  Parameters set by ckks_setup. curr_modulus_idx will be advanced by 1
@param[in,out] u      [Optional]. Ternary polynomial for asymmetric encryption. Can be
null if 'u' is stored in compressed form.
