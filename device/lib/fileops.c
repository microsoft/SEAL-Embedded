...
/**
Reads bytes stored at the specified location. On the Azure A7, this call called the
"image".
...
void load_pki(size_t i, const Parms *parms, ZZ *pki)
{
    ...
    #ifdef SE_NTT_NONE
    snprintf(fpath, MAX_FPATH_SIZE, "%s/pk%zu_%zu_%" PRIuZZ ".dat", SE_DATA_PATH, i, n,
             q);
    #else
    snprintf(fpath, MAX_FPATH_SIZE, "%s/pk%zu_ntt_%zu_%" PRIuZZ ".dat", SE_DATA_PATH, i,
             n, q);
    #endif
...
void load_ifft_roots(size_t n, double complex *ifft_roots)
{
    ...
    #ifdef SE_DATA_FROM_CODE_COPY
    double *ifft_roots_double = (double *)(ifft_roots);
    // -- This could be written more simply, but want to make clear
    //    that we are storing only 1 double at a time
    for (size_t i = 0; i < 2 * n; i += 2)
    {
        ifft_roots_double[i]     = (double)ifft_roots_save[i];
        ifft_roots_double[i + 1] = (double)ifft_roots_save[i + 1];
    }
    #elif defined(SE_DATA_FROM_CODE_DIRECT)
    SE_UNUSED(n);
...
void load_ntt_roots(const Parms *parms, ZZ *ntt_roots)
{
    ...
    ZZ q = parms->curr_modulus->value;
    char fpath[MAX_FPATH_SIZE];
    snprintf(fpath, MAX_FPATH_SIZE, "%s/ntt_roots_%zu_%" PRIuZZ ".dat", SE_DATA_PATH, n,
             q);
...
void load_intt_roots(const Parms *parms, ZZ *intt_roots)
{
    ...
    ZZ q = parms->curr_modulus->value;
    char fpath[MAX_FPATH_SIZE];
    snprintf(fpath, MAX_FPATH_SIZE, "%s/intt_roots_%zu_%" PRIuZZ ".dat", SE_DATA_PATH, n,
             q);
...
void load_ntt_fast_roots(const Parms *parms, MUMO *ntt_fast_roots)
{
    ...
    ZZ q = parms->curr_modulus->value;
    char fpath[MAX_FPATH_SIZE];
    snprintf(fpath, MAX_FPATH_SIZE, "%s/ntt_fast_roots_%zu_%" PRIuZZ ".dat", SE_DATA_PATH,
             n, q);
...
void load_intt_fast_roots(const Parms *parms, MUMO *intt_fast_roots)
{
    ...
    ZZ q = parms->curr_modulus->value;
    char fpath[MAX_FPATH_SIZE];
    snprintf(fpath, MAX_FPATH_SIZE, "%s/intt_fast_roots_%zu_%" PRIuZZ ".dat",
             SE_DATA_PATH, n, q);
