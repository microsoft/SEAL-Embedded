...
Timer struct and timing functions. Mainly useful for testing. We leave it as part of the
library in case we want to time internal functions.
*/
...
@param timer_instance  Timer instance (Used only with SE_ON_NRF5). Equal to
NRFX_TIMER_INSTANCE(0)
@param timer_config    Timer config (Used only with SE_ON_NRF5). Equal to
