...
The input is a polynomial of degree n in R_q, where n is assumed to be a power
of 2 and q is a prime such that q = 1 (mod 2n). The output is a vector A such
that the following hold: A[j] =  a(psi**(2*bit_reverse(j) + 1)), 0 <= j < n. For
more details, see the SEAL-Embedded paper and Michael Naehrig and Patrick Longa's paper.
*/
...
Space req: 'ntt_roots' should have space for 2n ZZ elements if SE_NTT_FAST is defined or n
ZZ elements if SE_NTT_ONE_SHOT or SE_NTT_REG is defined.
...
/**
Polynomial multiplication for inputs already in NTT form. 'res' and 'a' may share the same
starting address (see: poly_mult_mod_ntt_form_inpl)
...
*/
static inline void poly_mult_mod_ntt_form(const ZZ *a, const ZZ *b, size_t n,
...
@param[in]     mod  Modulus
*/
static inline void poly_mult_mod_ntt_form_inpl(ZZ *a, const ZZ *b, size_t n,
                                               const Modulus *mod)
{
