...
/**
Sets up a CKKS secret key. For symmetric encryption, this should either be
called once at the start during memory allocation (if storing s persistantly in
working memory) or once per encode-encrypt sequence (between base and remaining
steps, if not storing s persistantly in working memory).

If parms.sample_s == 1, will internally generate the secret key (from the uniform ternary
distribution). This is mainly useful for testing. If parms.sample_s == 0, will read in
...
@param[in,out] prng   [Optional] PRNG instance needed to generate randomness for secret
key polynomial. Should not be shared.
@param[out]    s      Secret key polynomial. Must have space for n coefficients. If
'small' s is used, this must be 2 bits per coefficient. Otherwise, this must be sizeof(ZZ)
per coefficient.
*/
void ckks_setup_s(const Parms *parms, uint8_t *seed, SE_PRNG *prng, ZZ *s);

/**
Initializes values for a single full symmetric CKKS encryption. Samples the error (w/o
respect to any prime). Should be called once per encode-encrypt sequence (just after
ckks_encode_base).
...
@param[in,out] shareable_prng  PRNG instance needed to generate first component of
ciphertexts. Is safe to share.
@param[in,out] prng            PRNG instance needed to generate error polynomial. Should
not be shared
...
/**
Encodes and symmetrically encrypts a vector of values using CKKS for the current modulus
prime.

Internally converts various objects to NTT form. If debugging encryption-only (assuming a
message of 0), or if calling to generate a public key, can set conj_vals_int to zero. In
this case, must set ep_small to the compessed form of the error.
...
@param[in,out] shareable_prng  PRNG instance needed to generate first component of
ciphertexts. Is safe to share.
@param         ntt_pte         Scratch space. Will be used to store plaintext plus error
in NTT form.
@param         ntt_roots       Scratch space. May be used to load NTT roots.
@param[out]    c0_s            1st component of the ciphertext. Stores n coefficients of
size ZZ.
@param[out]    c1              2nd component of the ciphertext. Stores n coefficients of
size ZZ.
...
/**
Updates parameters to next prime in modulus switching chain for symmetric CKKS encryption.
Also converts secret key polynomial to next prime modulus if used in expanded form
(compressed form s will be reduced later).
