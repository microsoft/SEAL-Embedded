...
/**
Wrapper struct for Public key. Required to keep track of whether public key is in NTT
form.
...
*/
static inline std::uint32_t upper32(std::uint64_t val)
{
...
In order to generate the public key, the key generator must read in the secret
key from file. This file may either be: 1) the file created using
SEAL-Embedded's adapter save functionality (SEAL-Embedded form). In this case,
set use_seal_sk_fpath to 0 and set sk_fpath to the path to the secret key. 2)
the file created using SEAL's stream save functionality (SEAL form). In this
case, set use_seal_sk_fpath to 1 and set seal_sk_fpath to the path to the secret
key.

@param[in] dirpath             Path to the directory to store file containing public key
in SEAL-Embedded form
...
@param[in] context             SEAL context
@param[in] use_seal_sk_fpath   If true, use seal_sk_fpath to load secret key.  Else, use
sk_fpath
*/
...
/**
Generates and saves the IFFT roots to file.

@param[in] dirpath          Path to the directory to store file containing public key in
SEAL-Embedded form
@param[in] context          SEAL context
@param[in] high_byte_first  If true, saves in (??)endian form
@param[in] string_roots
*/
void gen_save_ifft_roots(std::string dirpath, const seal::SEALContext &context,
                         bool high_byte_first, bool string_roots);

/**
Generates and saves the NTT roots to file.

@param[in] dirpath          Path to directory to store file containing NTT roots in
SEAL-Embedded form
@param[in] context          SEAL context
...
@param[in] high_byte_first  If true, saves in (??)endian form // TODO:
@param[in] string_roots
*/
...
/**
Generates and saves the index map (i.e. pi-inverse) to file. Also generates a code file
with hard-coded index map values.
